/* =========================
   BLACKWOOD - APP
   (Catalog + Cart + Checkout + NP Suggestions)
========================= */
      ...
      <article class="card">
        <div class="cardMedia">
          <img src="${p.img}" alt="${escapeHtml(p.title)}"
               onerror="this.onerror=null; this.src='img/products/core-3kg.png';">
        </div>
        <div class="cardBody">
    ...
    timer = setTimeout(async ()=>{
      try{
        ...
        listEl.innerHTML = items.map(x=>
          `<button type="button" data-val="${escapeHtml(x.value)}" data-id="${escapeHtml(x.id||"")}">${escapeHtml(x.label)}</button>`
        ).join("");
...
function renderCheckout(){
  ...
  const cart = getCart();
  totalEl.textContent = `${cartTotal(cart)} грн`;
  ...
  segBtns.forEach(b=> b.addEventListener("click", ()=>setDelivery(b.getAttribute("data-delivery"))) );
  setDelivery("branch");
  ...
  setupSuggest(whEl, whList, async (q)=>{
    ...
    const isPostomat = deliveryType === "postomat";
    ...
    // легкий фильтр по тексту (поштоматы обычно содержат "Поштомат")
    if(isPostomat){
      filtered = data.filter(x => String(x.Description||"").toLowerCase().includes("поштомат"));
    }else{
      filtered = data.filter(x => !String(x.Description||"").toLowerCase().includes("поштомат"));
    }
  ...
  sendBtn.addEventListener("click", ()=>{
    ...
    if(!fio || !phone || !city){ alert("Заполните ФИО, телефон и город."); return; }
